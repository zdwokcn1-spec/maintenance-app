...
# ================================================================
    ...
    if not df.empty:
        ...
        if st.session_state["logged_in"]:
            ...
            # ラベル生成時にNaTを考慮
            df['label'] = [(r['最終点検日'].strftime('%Y-%m-%d') if pd.notnull(r['最終点検日']) else "不明") + f" | {r['設備名']}" for _, r in df.iterrows()]
...
# ================================================================
    ...
    with st.form("reg", clear_on_submit=True):
        ...
        if st.form_submit_button("保存"):
            ...
            new_r = pd.DataFrame([{
                "設備名": f"[{en}] {ed}", 
                "最終点検日": wt.strftime('%Y-%m-%d'), # ここで文字列として保存
                "作業内容": wd, 
                "費用": wc, 
                "備考": wn, 
                "画像": b1 or "", 
