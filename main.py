    ...
    if not st.session_state["logged_in"]:
        ...
        if st.button("編集モードでログイン"):
            ...
            try:
                ...
                st.error("認証設定（Secrets）が見つかりません。")
    else:
        st.success("✅ 編集モード：有効")
        if st.button("ログアウト"):
            st.session_state["logged_in"] = False
...
if st.session_state.active_tab == "📊 ダッシュボード":
    ...
    # 日付が有効なデータのみ抽出
    df_valid = df.dropna(subset=['最終点検日'])
    
    if not df_valid.empty:
        st.subheader("📅 集計期間指定")
        col_d1, col_d2 = st.columns(2)
        min_date = df_valid['最終点検日'].min().date()
        max_date = df_valid['最終点検日'].max().date()
        start_date = col_d1.date_input("開始日", min_date)
        end_date = col_d2.date_input("終了日", max_date)
        
        mask = (df_valid['最終点検日'].dt.date >= start_date) & (df_valid['最終点検日'].dt.date <= end_date)
        f_df = df_valid.loc[mask].copy()
        ...
        if not f_df.empty:
            ...
            with c2:
                ...
                ax2.set_ylim(bottom=0, top=int(max_val * 1.2) + 1)
...
# 📁 過去履歴
    ...
    if not df.empty:
        ...
        for i, row in s_df.iterrows():
            # 日付のフォーマットエラー対策
            dt_display = row['最終点検日'].strftime('%Y-%m-%d') if pd.notnull(row['最終点検日']) else "日付未設定"
        ...
        if st.session_state["logged_in"]:
            ...
            df['label'] = df['最終点検日'].dt.strftime('%Y-%m-%d').fillna("未設定") + " | " + df['設備名'].astype(str)
            ...
            if st.button("🚨 この履歴を削除"):
                new_df = df.drop(idx).drop(columns=['label'])
...
# 📦 在庫管理
    ...
    if not stock_df.empty:
        ...
        if st.button(f"🗑️ {target_s} を削除"):
            new_stock = stock_df[stock_df["部品名"] != target_s]
    ...
    with st.form("reg", clear_on_submit=True):
        ...
        if st.form_submit_button("保存"):
            if not ed or not wd:
                ...
                st.success("完了しました！"); time.sleep(1); st.rerun()
