...
if df_raw.empty and stock_df_raw.empty:
    st.warning("データの読み込みに失敗しました。設定を確認後、ページを再読み込みしてください。")
    st.stop()
...
if st.session_state.active_tab == "📊 ダッシュボード":
    st.header("📊 メンテナンス状況概況"); filtered_df = get_filtered_data(df_main)
...
# 📁 過去履歴
    ...
    if not filtered_df.empty:
        ...
        if st.session_state["logged_in"]:
            ...
            if 'selected_record_idx' not in st.session_state: st.session_state.selected_record_idx = None

            def on_select_change():
                st.session_state.selected_record_idx = label_to_index_map.get(st.session_state.selectbox_key)

            selected_label = st.selectbox("修正・削除対象を選択", list(label_to_index_map.keys()), index=None, placeholder="修正したい履歴を選択してください", key="selectbox_key", on_change=on_select_change)
            
            idx_h = st.session_state.selected_record_idx
            ...
            if idx_h is not None:
                ...
                for i, part in enumerate(edited_parts_list):
                    ...
                    new_qty = p_col2.number_input(f"個数", value=part['個数'], min_value=1, step=1, key=f"edit_qty_{idx_h}_{i}")
                    edited_parts_list[i]['個数'] = new_qty
                    if p_col3.button(f"削除", key=f"del_part_{idx_h}_{i}"):
                        edited_parts_list.pop(i); st.rerun()
                ...
                if add_col2.button("部品を追加", key=f"add_part_btn_{idx_h}"):
                    if part_to_add != "-" and not any(p['部品名'] == part_to_add for p in edited_parts_list):
                        edited_parts_list.append({'部品名': part_to_add, '個数': 1}); st.rerun()
                
                st.markdown("---")
                
                # st.formで基本情報と最終的な部品リストを送信
                with st.form("edit_h_form"):
                    ...
                    new_note = cb.text_area("備考", str(curr_h["備考"]), height=100, key=f"edit_note_{idx_h}")
                ...
                if st.button("🚨 この履歴を削除", key=f"delete_button_{idx_h}"):
                    with st.spinner("データを削除し、在庫を更新中..."):
                        try:
                            used_parts = json.loads(curr_h['使用部品']) if curr_h['使用部品'] and curr_h['使用部品'] != '[]' else []
                        ...
                        except Exception as e: st.error(f"削除処理中にエラーが発生しました: {e}")
                    del st.session_state[f"edited_parts_{idx_h}"]
                    st.session_state.selected_record_idx = None
...
# 📦 在庫管理
    ...
    st.markdown("---"); st.subheader("🛠️ 在庫の修正・削除")
    if not stock_df.empty:
