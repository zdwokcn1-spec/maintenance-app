...
with tabs[1]:
    ...
    for i, row in df.sort_values(by="最終点検日", ascending=False).iterrows():
        ...
        with st.expander(f"{date_str} | {row['設備名']}"):
    ...
    if not df.empty:
        ...
        with st.form("edit_h_form_final"):
...
with tabs[2]:
    ...
    with st.expander("➕ 新しい部品を登録"):
        with st.form("new_stock_form"):
            ...
            nr = st.number_input("発注点", min_value=0, value=5)
            if st.form_submit_button("登録"):
                new_s = pd.DataFrame([{"分類": nc, "部品名": nn, "在庫数": nq, "単価": np, "発注点": nr, "最終更新日": datetime.now().strftime('%Y-%m-%d')}])
    ...
    if not f_items.empty:
        t_item = st.selectbox("修正する部品名", f_items["部品名"].tolist())
        ...
        with st.form("edit_stock_form"):
            ...
            er = st.number_input("発注点", value=int(s_row.get("発注点", 0)))
            if st.form_submit_button("在庫修正を保存"):
                stock_df.loc[stock_df["部品名"] == t_item, ["在庫数", "単価", "発注点", "最終更新日"]] = [eq, ep, er, datetime.now().strftime('%Y-%m-%d')]
...
with tabs[3]:
    ...
    with st.form("mainte_reg_final", clear_on_submit=True):
